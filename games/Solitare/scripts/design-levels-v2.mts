/**
 * design-levels-v2.mts — **오픈 확장(Expanding Frontier) 원리 기반 신규 레벨 생성기**(2026-07-19).
 *
 ...
 *   · composeLevelV2(shapeGrammar SKELETONS_V2 9종) — 커버 토폴로지가 트리/비늘(1→N fan-out)인 블록셀 조합.
 ...
 *
 * 사용: npx tsx scripts/design-levels-v2.mts [--write] [--from 1 --to 50]
 */
import fs from 'node:fs';
import { composeLevelV2, mulberry32 } from '../design/shapeGrammar.mjs';
...
/** 보드 예산 — 22 → 44 를 50레벨에 걸쳐 완만 점증(다양성 위주라 v1의 54장 상한보다 작게 유지). */
function budgetFor(level: number): number {
  return Math.min(44, 22 + Math.round(((level - 1) / (MAX_LEVEL_V2 - 1)) * 22));
}
...
const SKELETON_NAMES = ['비늘벽', '버스트아치', '쌍날개', '뭉치들판', '확장트리', '델타', '직조비늘', '크라운필드', '스타디움'];
...
function designLevel(lv: number): { doc: CardBoardDoc & Record<string, unknown>; report: string; metric: LevelMetricV2 } {
  ...
  const slots = composeLevelV2(lv, budget, mulberry32(lv * 2654435761 + 7));
  const doc = {
    schemaVersion: 1,
    kind: 'cardBoard',
    id: `v2-L${lv}`,
...
for (let lv = FROM; lv <= TO; lv++) {
  ...
  if (metric.minOpenAlongSolution >= 0 && metric.minOpenAlongSolution < 4) flagged.push(`lv${lv}: 중간최저 ${metric.minOpenAlongSolution}(<4)`);
  if (!metric.hasSolution) flagged.push(`lv${lv}: 해답 미확보`);
  if (metric.maxWave < metric.waves[0] + 2 && avgWave < 5) flagged.push(`lv${lv}: 확장·광폭 어느 쪽도 아님(웨이브 ${metric.waves.join('→')})`);
  if (bodyMin < 3) flagged.push(`lv${lv}: 단선 꼬리(웨이브 본체 최소 ${bodyMin} < 3)`);
  // 확장 이벤트: 광폭 유지형(뭉치들판 등 avgWave ≥5.5)은 면제 — 그 유형의 재미는 상시 넓은 선택지 자체.
  if (metric.expandEvents < 2 && avgWave < 5.5) flagged.push(`lv${lv}: 확장 이벤트 ${metric.expandEvents}회(<2)`);
