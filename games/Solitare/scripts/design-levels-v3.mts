/**
 * design-levels-v3.mts — **유기적 곡선 문법(v3) 기반 신규 레벨 생성기**(2026-07-19).
 *
 ...
 *   · composeLevelV3(shapeGrammar SKELETONS_V2 9종) — 커버 토폴로지가 트리/비늘(1→N fan-out)인 블록셀 조합.
 ...
 *
 * 사용: npx tsx scripts/design-levels-v3.mts [--write] [--from 1 --to 50]
 */
import fs from 'node:fs';
import { composeLevelV3, mulberry32 } from '../design/shapeGrammar.mjs';
...
const SKELETON_NAMES = ['화환', '쌍부채', '이중화환', '로제트꽃', '리본S', '가로스크롤', '날개묶음', '크라운호'];
...
function designLevel(lv: number): { doc: CardBoardDoc & Record<string, unknown>; report: string; metric: LevelMetricV2 } {
  ...
  const slots = composeLevelV3(lv, budget, mulberry32(lv * 2654435761 + 7));
  const doc = {
    schemaVersion: 1,
    kind: 'cardBoard',
    id: `v3-L${lv}`,
...
for (let lv = FROM; lv <= TO; lv++) {
  ...
  if (avgWave < 4.3) flagged.push(`lv${lv}: 프론티어 좁음(평균 웨이브 ${avgWave.toFixed(1)} < 4.3)`);
