...
// ─── dev 전용: 좀비 서비스워커 자동 제거 (store/Grillking vite.config 계승) ───
...
export default defineConfig({
  ...
  server: { port: 6196, host: true, strictPort: true },
  build: {
    target: 'es2020',
    sourcemap: true,
  ...
  plugins: [
    ...
    VitePWA({
      ...
      manifest: {
        name: '스모대전',
        short_name: '스모대전',
        description: '레인 디펜스 — 밀어내기 역사를 배치해 밀려오는 스모 군단을 막아내세요',
        lang: 'ko',
        display: 'standalone',
        display_override: ['standalone'],
        orientation: 'portrait',
        background_color: '#3D2419',
        theme_color: '#C0392B',
        start_url: './',
        scope: './',
      },
