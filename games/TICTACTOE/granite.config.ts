...
export default defineConfig({
  appName: 'tictactoe-neon', // TODO: 콘솔 등록명으로 교체
  brand: {
    displayName: '틱택토 네온',
    primaryColor: '#0A0714', // 게임 배경 네온 다크(index.html theme-color 와 동일)
    icon: '', // TODO: 콘솔 업로드 후 URL 기입
  },
  // 카메라·연락처·위치 등 네이티브 권한을 쓰지 않는다(광고는 권한 대상이 아님).
  permissions: [],
  web: {
    host: 'localhost',
    port: 6211,
   ...
   * 이 게임은 특히 **턴당 제한시간**이 있어서, 제스처로 화면이 새로고침되면 그 판이 통째로 날아간다.
   */
  webViewProps: {
    bounces: false, // iOS 고무줄 스크롤
    pullToRefreshEnabled: false, // 당겨서 새로고침(판 유실 방지)
