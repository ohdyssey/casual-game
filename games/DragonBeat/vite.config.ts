...
// ─── dev 전용: 좀비 서비스워커 자동 제거 (store/Grillking/SumoClash/Homerun vite.config 계승) ───
...
export default defineConfig({
  ...
  server: { port: 6198, host: true, strictPort: true },
  build: {
    target: 'es2020',
    sourcemap: true,
  ...
  plugins: [
    ...
    VitePWA({
      ...
      manifest: {
        name: '드래곤비트',
        short_name: '드래곤비트',
        description: '용선 리듬 레이스 — 북소리 박자에 맞춰 좌우로 노를 저어 결승선까지!',
        lang: 'ko',
        display: 'standalone',
        display_override: ['standalone'],
        orientation: 'portrait',
        background_color: '#0E7FA8',
        theme_color: '#18A0C9',
        start_url: './',
        scope: './',
      },
