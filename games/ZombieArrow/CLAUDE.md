...
| 캔버스 높이 모드 | 고정 2400 |
| 레이아웃 프레임 | 1080x2400(1개), 720x1280(3개) |
| 배경 노드 최대 폭 | 1351px |
| **채택 상태** | 🟡 **부분 적용 가능(배경 폭까지)** |

배경 폭이 1351px 이라 목표 상한(1600)까지는 못 늘립니다.
**배경이 덮는 만큼만** 부분 적용하면 에셋 재작업 없이 이득을 볼 수 있습니다.

```ts
// src/game.ts — designHeight 를 아래 두 줄로 교체
designHeightRange: { min: 2200, max: 2400 },
designWidthRange:  { min: 1080, max: 1351 },  // = 배경 최대 폭
```

이 설정이 여백 0으로 커버하는 최소 컨테이너 비율은 **1.63** 입니다.
- 커버됨: iPhone 15
