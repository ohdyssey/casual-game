/**
 * leaderboard — 양궁 글로벌 랭킹(1~10위) 데이터(순수 함수, 테스트 가능).
 *
 * 표시 규칙(공통 국기 라이브러리 참조): 행 = [순위][국기][아이디][점수].
 *   국가명이 아니라 "플레이어 아이디"를 노출한다(국기는 소속 국가 장식).
 *
 * 실시간 랭킹: NPC 9명의 점수는 판 시작 시 한 번 결정(고정 사다리, 내 아이디 시드).
 *   내 점수는 0에서 시작해 화살을 맞힐수록 오르고(만점 = ROUNDS×ARROWS×10), 매번 NPC 사다리에
 *   다시 끼워 등수를 매긴다 → 잘 쏠수록 왼쪽 패널에서 등수가 실시간으로 올라간다.
 *   - 내 식별자(아이디·국가)는 localStorage 에 1회 생성·영속(허브와 동일 키 → prod 동일 origin 공유).
 *
 * (ZombieArrow/Hub 의 leaderboard 와 설계 공유 — 추후 @casual/core 로 hoist 고려. 점수 스케일만 양궁용.)
 */
 ...
 * 양궁 만점은 작다(3R×3발×10 = 90). 최상위 ~78~88 에서 85~91%로 내려오는 사다리 →
 * 거의 완벽하게 쏴야 1위를 노릴 수 있고, 중하위권(~25~50)은 플레이하며 하나씩 추월한다.
 */
export function buildNpcScores(identity: Identity): NpcRow[] {
  ...
  let s = 78 + Math.floor(rand() * 11); // 최상위 78~88점
  return pool.map((n) => {
    const row: NpcRow = { iso3: n.iso3, id: n.id, score: Math.round(s) };
    s *= 0.85 + rand() * 0.06; // 다음 등수는 85~91%로 하강
    return row;
  });
}
